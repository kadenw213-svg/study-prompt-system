...
1. **Deadline pressure**: a non-optional deadline/exam within 48 h whose coverage has weak or untested concepts → that class, that coverage. Coverage = its `coverage_text` (via `modes/exam.md` rules); if blank, its `chapter_label`; if that's blank too, the chapters of the `weekly_overview` covering its due date.
2. **Old gaps**: earlier-than-current-week chapters that are Untested (never reached) or **stale** (`engine/route.md`: shaky *and* not tested in 14+ days), in any class. A class with **zero** Quiz evidence all term goes first. Then order by (a) prerequisite of this or next week's work, then (b) the oldest `last_tested_on`/week. → **Baseline Probe** (`engine/route.md`). Fluent → drop it from this rule for the session. Not fluent → teach + review-quiz it (a prerequisite goes before the dependent current work). **Interleave**: at most 2 consecutive rule-2 blocks, then rule 3 gets the next block if it has work, so a backlog of old chapters never stalls this week's material. A gap that is a prerequisite of this or next week's work is exempt and goes first.
3. **Current work, balanced**: this week's unmastered chapters. Pick the class with the fewest `blocks_today` (count 0 when `blocks_today_date ≠ today`; +1 per finished block), where a class with a non-optional deadline/exam in the next 7 days counts as 1 block fewer (it gets picked sooner). Ties → the nearest such deadline, then the oldest `last_studied_at`. Within the class, use normal routing (weak → in progress → unknown), preferring that deadline's coverage.
4. **Retention**: the class with the oldest `last_studied_at` → one batch at Applied/Edge on solid concepts, oldest `last_tested_on` first, preferring bank `niche`/repeat items. This is where old, well-known material comes back: a quick check, never a re-teach unless a question is missed.
